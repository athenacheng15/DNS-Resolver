...
# Buffer writers
# share the same message buffer and name-offset table, allowing names
# to be replaced with compression pointers to earlier positions in the packet.


def _write_question(message, question, name_offsets=None):
    _write_name(message, question.qname, name_offsets)
    message.extend(encode_uint16(question.qtype))
    message.extend(encode_uint16(question.qclass))


def _write_records(message, records, name_offsets=None):
    for record in records:
        _write_resource_record(message, record, name_offsets)


def _write_resource_record(message, record, name_offsets=None):
    _write_name(message, record.name, name_offsets)
    ...
    _write_rdata(message, record, name_offsets)
...
def _write_name(message, name, name_offsets=None):
    # Write a DNS name, optionally compressing it against earlier suffixes.
    # DNS name encoding is more complex because it must validate label and name
    # lengths, track suffix offsets, and generate compression pointers.
    if name == ".":
...
def _write_mx_rdata(message, rdata, name_offsets=None):
    message.extend(encode_uint16(rdata["preference"]))
    _write_name(message, rdata["exchange"], name_offsets)
...
# Standalone encoders
# create a buffer, call the required writers, and return the final bytes.


def encode_name(name):
    message = bytearray()
    _write_name(message, name)
    return bytes(message)


def encode_question(question):
    message = bytearray()
    _write_question(message, question)
    return bytes(message)
...
def encode_mx_rdata(rdata):
    message = bytearray()
    _write_mx_rdata(message, rdata)
    return bytes(message)


def encode_rdata(record):
    message = bytearray()
    _write_rdata(message, record)
    return bytes(message)


def encode_resource_record(record):
    message = bytearray()
    _write_resource_record(message, record)
    return bytes(message)


def encode_records(records):
    message = bytearray()
    _write_records(message, records)
    return bytes(message)


# Flags and message builders
...
def encode_dns_response(
    ...
    name_offsets = {}
    _write_question(message, question, name_offsets)
    _write_records(message, answers, name_offsets)
    _write_records(message, authorities, name_offsets)
    _write_records(message, additional, name_offsets)
